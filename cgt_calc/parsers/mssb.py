...
def __hacky_parse_decimal(decimal: str) -> Decimal:
    return Decimal(decimal.replace(",", ""))


def __init_from_release_report(row_raw: list[str], filename: str) -> BrokerTransaction:
    ...
    quantity = __hacky_parse_decimal(row["Net Share Proceeds"])
    price = __hacky_parse_decimal(row["Price"][1:])
...
def __init_from_withdrawal_report(
    ...
    price = __hacky_parse_decimal(row["Price"][1:])
    amount = __hacky_parse_decimal(row["Net Amount"][1:])
    ...
    for file in Path(transactions_folder).glob("*.csv"):
        ...
        with Path(file).open() as csv_file:
            ...
            if Path(file).name == "Withdrawals Report.csv":
                __validate_header(header, COLUMNS_WITHDRAWAL, str(file))
                transactions += [
                    __init_from_withdrawal_report(row, str(file)) for row in lines
                ]
            else:
                __validate_header(header, COLUMNS_RELEASE, str(file))
                transactions += [
                    __init_from_release_report(row, str(file)) for row in lines
                ]
